...
Every topic with its provenance (seed, keyword, both, emergent), size
...
verbatim member-paper snippets, or `--hops 2` to see the neighbourhood
...
The stored merge tree cut into as close to eight groups as it allows --
...
Both families' stored MCL partitions and the two disagreement lists:
...
The strongest chain over one family, hop by hop, each hop with its
...
The shared citekeys, both topic sizes, the corpus size, the
...
Pairwise shared papers, the papers held by all of the named topics, the
