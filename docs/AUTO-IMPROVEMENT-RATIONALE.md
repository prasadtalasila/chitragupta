...
| `prepare.py` + `evaluate_bpb` -- read-only ground truth | the five review aids, `chitragupta.draft gate`, #128's allowlist | The loop may run them and may not edit them |
...
- `findings` is what all five existing aids already emit.
