...
| `content/sync_plan.json` | **Absent after any completed run**, so the question does not arise: `sync` records the plan it resolved before parsing anything and discards it once every document in it has been attempted. Only an interrupted run leaves one, it carries a wall-clock `recorded_at`, and the only thing that ever reads it is the next `sync --resume` |
