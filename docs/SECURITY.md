...
Status: **reference.** Written 2026-09-01.
...
upload external analysis are commit-SHA pinned:
`softprops/action-gh-release`, `pypa/gh-action-pypi-publish`, and
`codecov/codecov-action`. GitHub-maintained actions currently use
major-version tags in these workflows, so not every action is SHA-pinned.
Codecov upload also depends on `secrets.CODECOV_TOKEN` in CI.
...
First, check the repository's GitHub **Security** page and any current
project-maintainer guidance for a private reporting channel. Use that
channel when it is available. If no private channel is published,
