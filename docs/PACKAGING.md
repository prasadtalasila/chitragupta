...
| `chitragupta draft dossier` | `init`, `status`, `mark-revision`, `sections`, `brief`, `set-language`, `acronyms-suggest`, `check-evidence`, `list`, `export`, `restore` |
...
That is 4 layers and 17 verbs and aids (3 + 9 + 4 + 1), plus 3
package-level commands, giving **41 invocable leaf commands**: 3 + 3 +
(4 + 23) + (3 + 4) + 1. The counts are stated because a table is easy to
