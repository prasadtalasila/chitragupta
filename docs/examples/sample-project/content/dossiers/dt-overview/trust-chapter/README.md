# Dossier: dt-overview/trust-chapter

The working state that produced `content/drafts/dt-overview/trust-chapter.tex` -- what a later
session needs in order to revise it without re-running the drafting
pipeline. Genre: thesis-chapter.
...
restore it with:
    ...
    python -m chitragupta.draft dossier export dt-overview/trust-chapter
