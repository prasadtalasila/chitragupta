...
| `bench_retrieval_token_floor.py` | Scores four token-length floors -- the shipped `len(w) > 2`, #790's proposed `> 1`, that same floor refusing a *newly admitted* bare number, and `> 0` -- over the same ground truths, with the keyword set swept down to its first 3, 2 and 1 keywords. Every figure is reported twice, over all queries and over the subset whose own terms the floor changes, because most queries carry no short word and averaging them in shrinks a real difference eightfold. Movement is counted over *all* queries even so: a lowered floor grows every document's token count, which is BM25's length-normalization denominator, so a query that never changed a term still moves. `--admitted` lists what each floor lets in, by document frequency, which is how the issue's premise that the floor only discards content words was checked rather than assumed. Both arms of every pair are written into the script, the shipped one included, and pinned against `retrieval._tokenize`/`_query_terms` in `self_check` |
