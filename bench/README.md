...
| `bench_retrieval_acronym_expansion.py` | Scores query-side acronym expansion (#789) at three weights under two vocabularies -- the vendored `assets/style/acronyms.toml` and a stand-in domain table read off the real book's own glossary -- against three ground truths. The two observed ones are **inert**: no query in either contains a vendored acronym, and a keyword list or a logged query that uses a domain one almost always spells it out too, which expansion deliberately never re-adds. The third set is the self-retrieval set with each expansion phrase rewritten to its acronym, **derived rather than observed** and labelled as such, because it is the only way to see the mechanism at all. One index serves every arm, which is the property being measured: expansion is query-side, so an unaffected query cannot move, and `self_check` asserts that rather than assuming it. Both vocabularies are written into the script, and the vendored one is checked against the shipped file so an edit there is a reason to re-measure rather than a silent change of arm |
