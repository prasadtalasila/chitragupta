...
| Does weighting a paper's **figure captions or table cells** above its body improve BM25 (#770)? | **`--only field-weights --field table`** on the same two scripts as the row above -- `FIELD_WEIGHT_GRID`'s `table` arms, both directions. `--field` is what keeps the committed record about the field the run is asking about: the grid is shared, so without it a sweep re-measures the title and abstract arms too and publishes numbers its own entry never discusses. `table` is **not a field chitragupta ships**: `arm_table_field` installs it into the scorer for the length of the run, with its own cold index, because the measurement declined it. There is no `caption` arm and cannot be one -- the corpus layer's passage sidecar carries no `caption` label (`_passage_records.PASSAGE_LABELS`), so the field would have an empty universe on every corpus. **Measured and declined** |
| Can a paper's **abstract** choose which passages the drafting layer is shown (#772's neighbourhood)? | **`bench_retrieval_abstract.py`** -- stdlib only, no GPU. Four mechanisms on the passage unit, scored on both ground truths: an abstract-score prior fused into every passage of its paper, an abstract-ranked paper shortlist, a query-independent lift by a passage's overlap with its own abstract, and dropping abstract passages outright. Reports `abstract_share@5` beside recall and nDCG, which is the row to read: it is what every gain here is made of. Also asks the prior question -- can an abstract *replace* full text as the paper ranker -- over all queries and over the subset whose own answer has an abstract, which is what separates "is an abstract a good enough representation" from "do enough papers have one". **Measured and declined** |
...
| `bench_retrieval_token_floor.py` | Scores four token-length floors -- `len(w) > 2` (the baseline, and what shipped until this measurement moved it), #790's proposed `> 1` (adopted, and what ships now), that same floor refusing a *newly admitted* bare number, and `> 0` -- over the same ground truths, with the keyword set swept down to its first 3, 2 and 1 keywords. Every figure is reported twice, over all queries and over the subset whose own terms the floor changes, because most queries carry no short word and averaging them in shrinks a real difference eightfold. Movement is counted over *all* queries even so: a lowered floor grows every document's token count, which is BM25's length-normalization denominator, so a query that never changed a term still moves. `--admitted` lists what each floor lets in, by document frequency, which is how the issue's premise that the floor only discards content words was checked rather than assumed. Both arms of every pair are written into the script and pinned against literals in `self_check`, not against `retrieval._tokenize` -- this is the script that proves why, since the floor moved *because of* its own tables, so a baseline reading the shipped tokenizer would have silently become the arm it was measuring against |
| `bench_retrieval_acronym_expansion.py` | Scores query-side acronym expansion (#789) off against on, under two vocabularies -- the vendored `assets/style/acronyms.toml` and a stand-in domain table read off the real book's own glossary -- against three ground truths. The two observed ones are **inert**: no query in either contains a vendored acronym, and a keyword list or a logged query that uses a domain one almost always spells it out too, which expansion deliberately never re-adds. The third set is the self-retrieval set with each expansion phrase rewritten to its acronym, **derived rather than observed** and labelled as such, because it is the only way to see the mechanism at all. One index serves every arm, which is the property being measured: expansion is query-side, so an unaffected query cannot move, and `self_check` asserts that rather than assuming it. An earlier revision swept a per-term weight at 0.25/0.5/1.0 -- full weight won every figure it moved, the setting became boolean, and those arms left with it; their figures stay in `results/2026-09-17-acronym-expansion/acronym_expansion_weight_sweep.json` and are not reproducible from this script as it stands. Both vocabularies are written into it, and the vendored one is checked against the shipped file so an edit there is a reason to re-measure rather than a silent change of arm |
...
and `bench_retrieval_abstract.py` each have
