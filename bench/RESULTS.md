...
| [2026-09-17 (#789): query-side acronym expansion](#2026-09-17-789-does-expanding-a-querys-acronyms-from-the-tier-1-vocabulary-find-papers-that-only-spell-the-term-out) | **Current, and it is a null result on the only vocabulary this project ships** | Zero of 256 self-retrieval queries and zero of 96 live-logged ones contain a vendored acronym, so every vendored arm is **bit-identical to the baseline** -- there is no measurement here that could support turning it on, and it ships **off**. On a derived set that rewrites each expansion phrase to its acronym (149 rows, 106 expanded), the mechanism works and is weight-monotone: recall@1 0.5660 -> 0.5943 and nDCG@5 0.6887 -> 0.7049 on the affected rows at weight 1.0, **8 queries better against 2 worse**, with recall@5 unmoved and the mean query-term document frequency up 264 -> 292 -- the precision cost, paid and visible. The observed sets are inert for a structural reason, not a sampling one: a keyword list or a logged query that uses an acronym nearly always spells it out too, and expansion never re-adds a word the caller typed |
...
query's terms at a configurable weight, so a query saying `DT` can reach
a paper that writes "digital twin" throughout and never abbreviates it.
Expansion *adds matches*, so #789 asks for precision beside recall and
names closing unmerged on a bad curve as a legitimate outcome.
...
Seven arms: expansion off, then three weights (0.25/0.5/1.0) under each
of two vocabularies. The **vendored** one is
...
Not "a small gain": zero queries were expanded, at every weight, so every
vendored row is the baseline's row to the last decimal. **That is the
whole argument for the shipped default being 0.0.** A default cannot be
justified by a measurement that had nothing to measure.
...
deliberately, since the added copy would otherwise rank the typed word at
the expansion weight. Same for the 96 logged queries, where a person
wrote "digital twin fidelity" rather than "DT fidelity". So both observed
sets say the same thing: **on real queries from this corpus, this feature
does nothing at all.**
...
the rewrite and kept; 106 of those are expanded by the arms. **Derived,
not observed** -- evidence about the mechanism and its cost, never about
how often the situation arises.

On the 106 expanded rows, before and after:

| arm | recall@1 | recall@5 | MRR@5 | nDCG@5 | mean query-term DF |
|---|---|---|---|---|---|
| expansion off | 0.5660 | 0.7925 | 0.6539 | 0.6887 | 264.0 |
| weight 0.25 | 0.5755 | 0.7925 | 0.6586 | 0.6922 | 292.1 |
| weight 0.5 | 0.5849 | **0.7830** | 0.6686 | 0.6976 | 292.1 |
| **weight 1.0** | **0.5943** | 0.7925 | **0.6753** | **0.7049** | 292.1 |

Over all 149 rows, with per-query movement against the baseline:

| arm | recall@1 | recall@5 | MRR@5 | nDCG@5 | better / worse |
|---|---|---|---|---|---|
| expansion off | 0.5839 | 0.8121 | 0.6740 | 0.7088 | -- |
| weight 0.25 | 0.5906 | 0.8121 | 0.6774 | 0.7113 | 1 / 0 |
| weight 0.5 | 0.5973 | 0.8054 | 0.6845 | 0.7151 | 6 / 1 |
| weight 1.0 | 0.6040 | 0.8121 | 0.6893 | 0.7203 | **8 / 2** |

Three things to read out of that pair of tables:

- **The gain is in rank, not in reach.** recall@5 does not move at 0.25
  or 1.0 and *falls* at 0.5 -- the added terms are mostly finding papers
  the query already reached and moving the right one up, which is what
  recall@1, MRR@5 and nDCG@5 all rising while recall@5 stands still
  means.
...
- **Weight 1.0 wins every column, which is not the safe reading it
  looks like.** At 1.0 a word the caller never typed counts exactly as
  much as one they did, and the 2 queries it makes worse are what that
  costs. The spread 0.25 -> 1.0 is monotone in every figure except
  recall@5, so the honest summary is "more weight, more of both", not
  "1.0 is optimal".

**The unaffected rows are a real control here, unlike in the token-floor
entry.** This change is query-side only: no document is re-tokenized, no
document length moves, and IDF is read from raw term frequencies, so a
query that gained no term cannot move by a float. Every `better`/`worse`
count above is a subset of the expanded rows, and `self_check` asserts
that -- if it ever fails, the harness is building a per-arm index and
measuring something else.

**Decision: ship, default off, at `[retrieval].acronym_expansion = 0.0`.**
The mechanism does what #789 asked for, and the seam is inert by
construction when it is off (`expand` returns before it loads a
vocabulary, and `bm25_scores` skips the multiplication entirely). But the
only vocabulary this project ships expands nothing in this corpus, so
there is no measurement that could justify on-by-default -- and the
benefit is a function of a per-host file, which is exactly the kind of
thing a default should not assume. A user with their own
`[style].acronyms` has the derived table above to start from; the right
number on their corpus is an empirical question this one cannot answer
for them.
...
field); whether expansion helps a *dense* retriever, which has no such
problem to begin with; and the reverse direction -- a query saying
"digital twin" still cannot reach a document that only writes `DT`,
because nothing here touches the index. #789 records that as a separate
question, and it is untouched by this run.
...
Record: `bench/results/2026-09-17-acronym-expansion/acronym_expansion.json`.
