...
Status: **proposed.** Written 2026-08-30. Tracked as
[#472](https://github.com/prasadtalasila/chitragupta/issues/472).
...
The change is what makes alignment mean anything. Under one-file-per-
section every file *is* a section by construction, so nothing can drift
and there is nothing to check. Only when a chapter is a single document
can its headings diverge from the structure a human approved.

It also matches the only book that exists. `digital-twins-for-software-
engineers` is one file per chapter, achieved by the retrofit giving each
chapter a single `####` section carrying the filename -- a workaround for
the shipped rule rather than the rule. This makes the real shape the
declared one.

**Where a chapter's draft lives:** `content/drafts/<book>/<chapter-id>.md`
(or `.tex`), named for the chapter's own `{#id}`. No fallback and no
search. A retrofitted chapter is never *section-described* (below), so
`align` never looks for its draft and the legacy naming never has to be
guessed at.

`unit contract`'s "`{id}` is a chapter, not a section" refusal has to
widen to accept a chapter. That is PR 3's, not PR 1's: `align` reads and
refuses nothing.
