...
16. **Run the verbatim scan.** Before presenting, rebuild the section map
    ...
    the connective prose between worked examples, which cites nothing. **A
    review aid, not a gate: it exits 0 either way, it cannot block the
    draft, and it is never a condition of presenting.** Show what it found
    rather than summarising it away, and lead with the `long` and `short`
    buckets -- a `quoted` run that also cites its source is a legitimate
    attributed quotation, so give those a count rather than a list.
