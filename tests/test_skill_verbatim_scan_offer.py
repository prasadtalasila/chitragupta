...
def test_genre_doc_still_speaks_for_every_skill_that_exists():
    """docs/GENRE.md says "all seven" in prose. Prose can't count.

    If an eighth skill lands, the shared-conventions section silently
    ...
    assert count == 7, (
        f"{count} skills exist but docs/GENRE.md still says \"all seven\". "
        "Update that section -- and check the new skill carries the gate, "
        "dossier and verbatim-scan conventions it states."
    )
    assert "What all seven have in common" in GENRE_DOC.read_text(encoding="utf-8")
