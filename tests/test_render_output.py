...
class TestMarkdownTwinWarnings:
    """A Markdown draft holds its ASCII twice -- inline in the fence, which
    is what the md render emits, and in figures/<name>.txt. Nothing in the
    render path reads the second copy, so it is checked instead of trusted."""

    def test_a_missing_twin_is_flagged(self, tmp_path):
        (tmp_path / "figures").mkdir()
        (tmp_path / "figures" / "fig1.tex").write_text(TIKZ_FIGURE)
        draft = tmp_path / "draft.md"
        warnings = render_output._figure_warnings(MARKED_FENCE, draft)
        assert any("no ASCII twin beside figures/fig1.tex" in w for w in warnings)

    def test_a_twin_that_has_drifted_from_the_fence_is_flagged(self, tmp_path):
        figure_pair(tmp_path)
        (tmp_path / "figures" / "fig1.txt").write_text(
            "  +-------+  read   +--------+\n  | model | ------> | SOLVER |\n"
        )
        draft = tmp_path / "draft.md"
        warnings = render_output._figure_warnings(MARKED_FENCE, draft)
        assert any("drifted from the fence" in w for w in warnings)

    def test_trailing_whitespace_alone_is_not_drift(self, tmp_path):
        # Invisible on the page, so reporting it would train a reader to
        # ignore the warning that matters.
        figure_pair(tmp_path)
        (tmp_path / "figures" / "fig1.txt").write_text(
            "\n" + ASCII_FIGURE.replace("+--------+\n", "+--------+   \n") + "\n\n"
        )
        draft = tmp_path / "draft.md"
        assert render_output._figure_warnings(MARKED_FENCE, draft) == []

    def test_the_twin_is_derived_from_the_one_marker(self, tmp_path):
        # One marker names the pair; a second marker naming the .txt would
        # be two references to keep in step.
        assert render_output._ascii_twin_ref("figures/fig1.tex") == "figures/fig1.txt"
