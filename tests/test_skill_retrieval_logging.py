...
(docs/TOKENS.md) into something `python3 -m src.dossier status` can total.
...
# `python3 -m src.retrieval search "..."` / `... evidence "..." --citekey ...`
# -- the CLI form, which is the only one `--log` exists on.
_INVOCATION = re.compile(r"python3 -m src\.retrieval (search|evidence)\b")
