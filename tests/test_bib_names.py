...
class TestBothCallersUseTheSharedGrammar:
    ...
    `bib_reader._parse_authors` and `references._format_name`, so nothing
    ...
    def test_references_initializes_exactly_what_the_helper_split(self, name):
        ...
        assert references._format_name(name) == f"{references._initials(first)} {last}".strip()
    ...
    def test_both_modules_agree_on_the_family_name(self, name):
        ...
        assert bib_reader._parse_authors(name)[0][1] in references._format_name(name)
    ...
    def test_extending_the_grammar_reaches_both_callers(self, monkeypatch):
        ...
        assert references._format_name("Doe, Jane") == "G. FAMILY"

    def test_a_corporate_author_does_not_reach_the_shared_grammar(self, monkeypatch):
        """The brace check stayed in `references`, and this pins that it
        still short-circuits: "{IEEE Standards Association}" is one unit,
        and splitting it would print "S. Association"."""
        monkeypatch.setattr(bib_names, "split_name", lambda name: ("GIVEN", "FAMILY"))
        assert references._format_name("{IEEE Standards Association}") == (
