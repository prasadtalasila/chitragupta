...
REVIEW_FLAT_AIDS = {"provenance", "coverage", "synthesis", "figure"}
...
class TestStatedCounts:
    """The formula docs/PACKAGING.md states in prose: 4 layers, 19 verbs
    and aids, 43 invocable leaf commands -- computed here from the same
    structures the tests above already verified against the live code,
    not retyped as a fresh set of literals."""

    def test_nineteen_verbs_and_aids(self):
        ...
        assert verbs_and_aids == 19
        assert "19 verbs and aids" in PACKAGING_TEXT

    def test_forty_three_leaf_commands(self):
        ...
        assert total == 43
        assert "43 invocable leaf commands" in PACKAGING_TEXT
