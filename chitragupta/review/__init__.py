...
`chitragupta/review/uncited_prose.py` and `chitragupta/review/claim_support.py`. Each reads a draft -- plus the
...
this module is what makes the seven obey one output contract instead of
seven.
...
`provenance` and `coverage` since #309, `synthesis`, `figure` and
`uncited` from the day each landed, and `support` the same way -- which is why
